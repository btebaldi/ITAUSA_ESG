...
PIB <- read_excel("./database/Tabela 1620.xlsx",
                  range = cell_limits(ul = c(5, 1), lr = c(NA, 2)))
colnames(PIB) <- c("Data", "PIB")
PIB$Data <- seq(from = as.Date("1996-01-01"), to = as.Date("2023-07-01"), by="quarter")

Luz <- read_excel("./database/luzipeadata[10-10-2023-08-14].xls",
...
DT |>
  ...
  scale_linetype_manual(values=c("solid", "dashed", "dotted"),
                        ...
                        labels=c("PIB real", "CO2", "Energia")) +
  scale_fill_manual(values=c("#339966", "#006600", "#FF9966", "#FF6600"),
                    breaks = c("B", "A", "D", "C"),
                    labels = c("Ganho ecologico", "Ganho da Produtividade de Energia",
                               "Perda ecologica", "Perda da Produtividade de Energia")) +
...
DT |>
  ggplot() + 
  geom_line(aes(x = year, y = GDP, linetype = "Real GDP", colour="GDP"), size = 1) +
  geom_line(aes(x = year, y = co2, linetype = "CO2", colour="CO2"), size = 1) +
  # geom_line(aes(x = year, y = KWH, linetype = "Energy"), size = 1) +
  ...
  scale_linetype_manual(values=c("solid", "twodash", "dotted"),
                        breaks = c("Real GDP", "CO2", "Energy"),
                        labels = c("PIB", "CO2", "Energia")) +
  scale_color_manual(values=c("black", "red", "dotted"),
                        breaks = c("GDP", "CO2", "Energy"),
                        labels = c("PIB", "CO2", "Energia")) +
  scale_fill_manual(values=c("#339966", "#006600", "#FF9966", "#FF6600"),
                    breaks = c("B", "A", "D", "C"),
                    labels = c("Ganho ecologico", "Ganho da Produtividade de Energia",
  ...
  theme(legend.position = "bottom") +
  labs(title = NULL,
       caption = "Both series are normalized to 100 at 1995",
       linetype = NULL, fill = NULL,
       y = NULL,
       x = NULL)

ggsave(filename = "PIB-by-Energy.png", scale = 1, width = 10.24, path ="Graficos", dpi = 100)
...
DT |>
  ...
  geom_line(aes(x = year, y = GDP, linetype = "Real GDP", colour="GDP"), size = 1) +
  # geom_line(aes(x = year, y = co2, linetype = "CO2", colour="CO2"), size = 1) +
  geom_line(aes(x = year, y = KWH, linetype = "Energy", colour="Energy"), size = 1) +
...
ggsave(filename = "PIB-by-CO2.png", scale = 1, width = 10.24, path ="Graficos", dpi = 100)
